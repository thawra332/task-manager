...
| Problem | Solution |
|---------|----------|
| **Port already in use** | Backend: Change PORT in .env file. Frontend: Use PORT=3001 npm start |
...
| **SQLITE_ERROR: no such table** | Delete `database.sqlite` and restart backend (table recreates automatically) |
...
3. Restart the backend (the file will be recreated with the tasks table)
...
2. Ensure frontend is making requests to http://localhost:5000

3. Check that no other service is blocking port 5000

## Environment Variables
Create ``` .env ``` in the ```bash backend/``` folder:
